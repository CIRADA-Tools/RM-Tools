...
def model(pDict, lamSqArr_m2):
    ...
    quArr = (pArr * np.exp(2j * (np.radians(pDict["psi0_deg"]) + (0.5*pDict["RM_src_radm2"] * lamSqArr_m2))) * (np.sin(pDict["RM_src_radm2"] * lamSqArr_m2) / (pDict["RM_src_radm2"] * lamSqArr_m2))    
    return quArr
