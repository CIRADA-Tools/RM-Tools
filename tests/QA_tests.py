...
class test_RMtools(unittest.TestCase):
    ...
    def test_a2_1D_synth_values(self):
        ...
        self.assertEqual(mDict['dFDFcorMAD'], 0.013049868866801262, 'dFDFcorMAD differs from expectation.')
        self.assertEqual(mDict['phiPeakPIchan_rm2'], 201.0, 'phiPeakPIchan_rm2 differs from expectation.')
        self.assertEqual(mDict['dPhiPeakPIchan_rm2'], 0.23030644244851126, 'dPhiPeakPIchan_rm2 differs from expectation.')
        self.assertEqual(mDict['ampPeakPIchan'], 0.6870054006576538, 'ampPeakPIchan differs from expectation.')
        self.assertEqual(mDict['ampPeakPIchanEff'], 0.686947275572337, 'ampPeakPIchanEff differs from expectation.')
        self.assertEqual(mDict['dAmpPeakPIchan'], 0.005892556709695188, 'dAmpPeakPIchan differs from expectation.')
        self.assertEqual(mDict['peakFDFimagChan'], -0.5500036478042603, 'peakFDFimagChan differs from expectation.')
        self.assertEqual(mDict['peakFDFrealChan'], 0.4116702973842621, 'peakFDFrealChan differs from expectation.')
        self.assertEqual(mDict['dPolAngleChan_deg'], 0.24571759529389078, 'dPolAngleChan_deg differs from expectation.')
        self.assertEqual(mDict['Ifreq0'], 0.9884242189264847, 'Ifreq0 differs from expectation.')
        ...
        self.assertEqual(mDict['lam0Sq_m2'], 0.10327484831236765, 'lam0Sq_m2 differs from expectation.')
        ...
        self.assertEqual(mDict['fracPol'], 0.6952781325679147, 'fracPol differs from expectation.')
        self.assertEqual(mDict['sigmaAddQ'], 0.024110774950646455, 'sigmaAddQ differs from expectation.')
        self.assertEqual(mDict['sigmaAddU'], 0.022015690059319505, 'sigmaAddU differs from expectation.')
    ...
    def test_b2_1D_synth_values(self):
        ...
        self.assertEqual(mDict['dAmpObserved'], 0.004037752747535706, 'dAmpObserved differs from expectation.')        
    ...
    def test_f2_1D_synth_values(self):
        ...
        self.assertTrue(abs(mDict['chiSqRed']-0.8739606950666521)/0.8739606950666521 < 0.01 , 'chiSqRed differs from expectation.')
        self.assertTrue(abs(mDict['BIC']+546.5056488755044)/546.50564887550441 < 0.001 , 'BIC differs from expectation.')
