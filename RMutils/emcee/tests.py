...
def log_unit_sphere_volume(ndim):
    if ndim % 2 == 0:
        ...
        for i in range(1, ndim + 1, 2):
            logfactorial += np.log(i)
        return (
            (ndim + 1) / 2.0 * np.log(2.0)
            + (ndim - 1) / 2.0 * np.log(np.pi)
            - logfactorial
        )
...
class Tests:
    def setUp(self):
        ...
        self.cov = 0.5 - np.random.rand(self.ndim**2).reshape((self.ndim, self.ndim))
    ...
    def check_pt_sampler(self, cutoff, N=None, p0=None):
        ...
        assert (
            np.mean(self.sampler.acceptance_fraction) > 0.1
        ), "acceptance fraction < 0.1"
        assert (
            np.mean(self.sampler.tswap_acceptance_fraction) > 0.1
        ), "tswap acceptance frac < 0.1"

        maxdiff = 10.0**logprecision

        chain = np.reshape(
            self.sampler.chain[0, ...], (-1, self.sampler.chain.shape[-1])
        )

        # np.savetxt('/tmp/chain.dat', chain)

        log_volume = (
            self.ndim * np.log(cutoff)
        ...
        assert np.abs(lnZ - (gaussian_integral - log_volume)) < 3 * dlnZ, (
            "evidence incorrect: {0:g} versus correct {1:g} (uncertainty " "{2:g})"
        ).format(lnZ, gaussian_integral - log_volume, dlnZ)
        assert np.all(
    ...
    def test_nan_lnprob(self):
        self.sampler = EnsembleSampler(
            self.nwalkers, self.ndim, lnprob_gaussian_nan, args=[self.icov]
        )
    ...
    def test_parallel(self):
        self.sampler = EnsembleSampler(
            self.nwalkers, self.ndim, lnprob_gaussian, args=[self.icov], threads=2
        )
        self.check_sampler()
    ...
    def test_pt_sampler(self):
        ...
        p0 = np.random.multivariate_normal(
            mean=self.mean, cov=self.cov, size=(self.ntemp, self.nwalkers)
        )
    ...
    def test_blobs(self):
        ...
        assert (
            self.sampler.chain.shape == (self.nwalkers, self.N, self.ndim)
